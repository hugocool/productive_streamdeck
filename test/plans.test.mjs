...
test('planStashPush adds a stack entry and builds steps', () => {
  ...
  const { plan, next } = planStashPush({ kind: 'visible' }, snapshot, prev, 'STASH', 'blank-');
  ...
  assert.deepEqual(plan.steps[3].args, ['summon-workspace', 'blank-1']);
});

test('planStashPop applies and drops the top entry', () => {
  const prev = emptyStashStack();
  const pushed = planStashPush({ kind: 'visible' }, snapshot, prev, 'STASH', 'blank-').next;
